    ...
    <div class="space-y-6 animate-fade-in">
      ...
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-sm text-slate-500 mt-0.5">Registra tus predicciones antes del inicio</p>
        </div>
        <button (click)="matchFacade.loadMatches()"
      ...
      @if (matchFacade.error()) {
        <div class="card card-body flex items-center gap-3 text-red-600 bg-red-50 border-red-200">
        ...
        <section>
          ...
          <div class="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
            @for (match of group[1]; track match.id) {
              <div class="card card-hover cursor-pointer transition-all duration-200
                          hover:-translate-y-0.5 animate-slide-up"
                   [class.opacity-75]="match.status === 'Finished'">
                <div class="card-body">
                  ...
                  <div class="flex items-center justify-between gap-2">
                    ...
                    <div class="flex flex-col items-center gap-1.5 flex-1 min-w-0">
                      <div class="w-10 h-10 rounded-xl bg-gradient-primary/10 border
                                  border-primary-100 flex items-center justify-center text-sm font-bold
                                  text-primary-700">
                        {{ teamInitial(match.homeTeamName) }}
                      </div>
                      <span class="text-xs font-semibold text-slate-700 text-center leading-tight truncate w-full">
                        {{ match.homeTeamName }}
                      </span>
                    </div>
                    ...
                    <div class="flex flex-col items-center gap-1 shrink-0">
                      <span class="text-xs font-bold text-slate-400 tracking-widest">VS</span>
                  ...
                  <!-- Round -->
                  <p class="text-xs text-slate-400 text-center mt-2">{{ match.roundName }}</p>

                  <!-- Action button -->
                  <div class="mt-3 pt-3 border-t border-slate-100">
                    @if (match.status === 'Finished') {
                      <div class="text-center">
                        <span class="text-xs text-slate-400">Partido finalizado</span>
                      </div>
                    } @else {
                      <button
                        (click)="openPrediction(match)"
                        class="btn-primary w-full btn-sm">
                        {{ getPrediction(match.id) ? '✏ Editar predicción' : '+ Predecir' }}
                      </button>
                    }
                  </div>
      ...
      <!-- Empty state -->
      @if (!matchFacade.loading() && !matchFacade.error() && matchFacade.matches().length === 0) {
        <div class="card card-body text-center py-16">
          <span class="text-5xl mb-4 block">🏟</span>
          <h3 class="font-semibold text-slate-700">Sin partidos disponibles</h3>
          <p class="text-sm text-slate-500 mt-1">Los partidos se cargarán próximamente.</p>
        </div>
      }
...
export class MatchListComponent implements OnInit {
  ...
  getPrediction(matchId: number) {
    return this.predictionFacade.predictions().find(p => p.idMatch === matchId) ?? null;
  }
