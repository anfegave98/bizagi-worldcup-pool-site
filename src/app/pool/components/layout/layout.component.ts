  ...
  template: `
    <div class="min-h-screen bg-slate-50 flex flex-col">
      ...
      <header class="glass sticky top-0 z-40">
        <div class="max-w-7xl mx-auto px-4 sm:px-6">
          <div class="flex items-center justify-between h-16">
            ...
            <a routerLink="/app/matches"
              ...
              <div class="w-9 h-9 rounded-xl bg-gradient-primary shadow-md flex items-center
                          justify-center text-lg transition-transform group-hover:scale-105">
                ⚽
              </div>
              <span class="font-bold text-slate-800 hidden sm:block">Polla Mundialista</span>
            </a>

            <!-- Desktop nav -->
            <nav class="hidden md:flex items-center gap-1">
              @for (item of visibleNav(); track item.route) {
                <a [routerLink]="item.route" routerLinkActive="bg-primary-50 text-primary-700"
            ...
            <div class="flex items-center gap-3">
              ...
              <button (click)="menuOpen.set(!menuOpen())"
                ...
                <div class="avatar w-8 h-8 text-xs bg-gradient-primary shadow-sm">
                  {{ initials() }}
                </div>
              ...
              <!-- Dropdown -->
              @if (menuOpen()) {
                <div class="absolute top-14 right-4 w-52 card shadow-card-lg animate-scale-in z-50">
        ...
        <!-- Mobile nav -->
        @if (mobileOpen()) {
          <div class="md:hidden border-t border-slate-100 bg-white px-4 py-3
...
export class LayoutComponent {
  ...
  visibleNav = () => this.navItems.filter(n =>
    !n.adminOnly || this.facade.isAdmin()
  );
