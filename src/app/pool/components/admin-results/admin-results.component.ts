...
@Component({
  ...
  template: `
    <div class="space-y-6 animate-fade-in">
      ...
      <!-- Header -->
      <div>
        <h2 class="text-xl font-bold text-slate-800">Registrar Resultados ⚙️</h2>
        <p class="text-sm text-slate-500 mt-0.5">
          Registra el resultado real de cada partido. Se calculan los puntos automáticamente.
        </p>
      </div>
              ...
              <div class="card card-hover animate-slide-up">
                <div class="card-body">
                  <div class="flex flex-col sm:flex-row sm:items-center gap-4">
                    ...
                    <div class="flex items-center gap-4 flex-1">
                      <div class="flex items-center gap-3 flex-1 min-w-0">
                        <div class="flex flex-col items-center gap-1 min-w-0">
                          <div class="w-10 h-10 rounded-xl bg-gradient-primary/10 border border-primary-100
                                      flex items-center justify-center text-xs font-bold text-primary-700">
                            {{ initial(match.homeTeamName) }}
                          </div>
                          <span class="text-xs font-medium text-slate-700 text-center truncate max-w-[64px]">
                            {{ match.homeTeamName }}
                          </span>
                        </div>
                        ...
                        <div class="flex flex-col items-center gap-1 min-w-0">
                          <div class="w-10 h-10 rounded-xl bg-violet-50 border border-violet-100
                    ...
                    <!-- Result form or register button -->
                    @if (selectedMatchId() === match.id) {
                      <div class="flex items-center gap-2 flex-wrap">
                        <input [formControl]="getForm(match.id).controls.homeGoals"
                          type="number" min="0" placeholder="L"
                          class="input w-16 text-center font-bold" />
                        <span class="text-slate-400 font-bold">–</span>
                        <input [formControl]="getForm(match.id).controls.awayGoals"
                          type="number" min="0" placeholder="V"
                          class="input w-16 text-center font-bold" />
                        <button (click)="submitResult(match)"
                        ...
                        <button (click)="selectedMatchId.set(null)" class="btn-ghost btn-sm">
                          Cancelar
                        </button>
                      </div>
                    } @else {
              ...
              <div class="card opacity-70">
                <div class="card-body py-3">
                  <div class="flex items-center gap-3">
                    <span class="badge badge-success shrink-0">✓ Finalizado</span>
      ...
      <!-- All finished -->
      @if (!matchFacade.loading() && matchFacade.scheduled().length === 0 && matchFacade.finished().length > 0) {
        <div class="card card-body text-center py-12">
          <span class="text-4xl mb-3 block">🎉</span>
          <h3 class="font-semibold text-slate-700">Todos los partidos tienen resultado</h3>
          <p class="text-sm text-slate-500 mt-1">Los puntos ya fueron calculados.</p>
        </div>
      }
...
export class AdminResultsComponent implements OnInit {
  ...
  private buildForm() {
    return this.fb.group({
      homeGoals: [null as number | null, [Validators.required, Validators.min(0)]],
      awayGoals: [null as number | null, [Validators.required, Validators.min(0)]],
    });
  }
  ...
  submitResult(match: MatchDto): void {
    ...
    this.adminFacade.registerResult(match.id, {
      homeGoals: Number(homeGoals),
      awayGoals: Number(awayGoals),
    }).subscribe({
      next: () => {
        this.toast.success(`Resultado registrado: ${match.homeTeamName} ${homeGoals} – ${awayGoals} ${match.awayTeamName}. ¡Puntos calculados!`);
